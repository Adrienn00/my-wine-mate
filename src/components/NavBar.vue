<template>
  <header :class="headerClass">
    <div class="flex min-h-[82px] flex-col lg:flex-row">
      ...
      >
        <BaseButton to="/" variant="homepage">Vinology Cook</BaseButton>
        <span
      ...
      <div class="flex-1 bg-[linear-gradient(180deg,#5d1f32_0%,#4a1426_100%)] text-[#f9ead7]">
        <div class="flex flex-col gap-3 px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
          <nav class="flex flex-wrap items-center gap-1.5 font-semibold">
            ...
            <BaseButton v-if="route.path !== '/foodPairing'" to="/foodPairing" variant="navLink">Pairing</BaseButton>
            <BaseButton
          ...
          <div class="flex flex-wrap items-center gap-2">
            <template v-if="!['/login', '/signup', '/user'].includes(route.path)">
              <template v-if="isLoggedIn">
                ...
                <BaseButton to="/profile" variant="navAccent">Profile</BaseButton>
...
const headerClass = computed(() => 'sticky top-0 z-40 overflow-hidden border-b border-[rgba(95,40,53,0.12)] backdrop-blur-sm')
